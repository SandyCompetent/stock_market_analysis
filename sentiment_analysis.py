    ...
    def __init__(self, model_name="ProsusAI/finbert"):
        ...
        try:
            ...
            if self.pos_id is None or self.neg_id is None:
                raise KeyError("Model labels do not include 'positive' and/or 'negative'.")
...
def aggregate_daily_sentiment(sentiment_df):
    if sentiment_df is None or sentiment_df.empty: return None
    print("\\n📊 Aggregating daily sentiment scores...")
    daily_sentiment = sentiment_df.groupby('Date').agg({'Sentiment_Score': ['mean', 'sum', 'count']}).round(4)
    daily_sentiment.columns = ['Avg_Sentiment', 'Total_Sentiment', 'News_Count']
