...
def plot_enhanced_diagnostics(y_test, predictions, test_dates, stock_symbol, model_name):
    """
    ...
    fig.suptitle(f'{stock_symbol} - {model_name} Enhanced Diagnostic Analysis', 
    ...
    # Save the plot
    safe_model_name = model_name.replace(" ", "_").replace("(", "").replace(")", "")
    save_path = os.path.join(cfg.OUTPUT_DIR, f"{stock_symbol}_{safe_model_name}_enhanced_diagnostics.png")
...
def plot_model_comparison_metrics(results_df, stock_symbol):
    ...
    fig.suptitle(f'{stock_symbol} - Model Performance Comparison', fontsize=16, fontweight='bold')
    ...
    for i, metric in enumerate(metrics):
        ...
        # Create bar plot
        bars = axes[i].bar(range(len(models)), values, color=colors, alpha=0.8, edgecolor='black')
        ...
        for j, (bar, value) in enumerate(zip(bars, values)):
            ...
            axes[i].text(bar.get_x() + bar.get_width()/2., height + height*0.01,
                        f'{value:.4f}' if metric != 'MAPE (%)' and metric != 'Directional_Accuracy' 
                        else f'{value:.2f}%' if metric == 'MAPE (%)' or metric == 'Directional_Accuracy'
                        else f'{value:.4f}',
    ...
    # Save the plot
    save_path = os.path.join(cfg.OUTPUT_DIR, f"{stock_symbol}_model_comparison_metrics.png")
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
...
def plot_top_models_comparison(results_dict, stock_symbol, top_n=4):
    ...
    if actual_data:
        plt.plot(actual_data["dates"], actual_data["values"], 
                label="Actual Price", color="black", linewidth=3, alpha=0.8)
    ...
    plt.title(f'{stock_symbol} - Top {top_n} Model Predictions vs Actual Prices', 
              fontsize=16, fontweight='bold')
    ...
    textstr = f'Comparison Period: {len(actual_data["dates"]) if actual_data else "N/A"} days'
    ...
    # Save the plot
    save_path = os.path.join(cfg.OUTPUT_DIR, f"{stock_symbol}_top_models_comparison.png")
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
